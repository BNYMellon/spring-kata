...
The `@Service` is an alias for the
`@Component`annotation (see the code). It should display a
`@AliasFor(annotation = Component.class)`.
The service implementation auto-wires a
...
A new dependency `hibernate-entitymanager` is added.
...
The *DAO* layer declares an `EntityManager`, as a `@PersistenceContext`.

The transaction management is performed via the annotation `@Transactional`.
The DAO is declared as _read-only_ as default for the entire class and as _read-write with propagation_ for the
`save` method.
