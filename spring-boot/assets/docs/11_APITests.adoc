...
REST operations can be tested with several third party libraries.
The examples use Spring's `MockMvc` as a framework of choice for testing the RESTful layer.
...
.. The second class annotation declares that this test is a `@WebMvcTest`.
Using this annotation prevents most auto-configurations, and simply loads configurations limited to the Model-View-Controller (MVC) layer, such as Controllers and not any `@Service`, `@Repository` or even `@Component` annotations.
...
Testing the application manually, requires launching the application.
The application can be launched from: either the IDE (run the TodoApplication).
...
****
*Description*: Fetch all *active* todos by assignee, when assignee is omitted.
...
****
*Description*: Fetch all *active* todos by assignee, when assignee value is too short.
Should respond with an error message. +
*HTTP Method type*: *GET* +
...
****
*Description*: Fetch all *active* todos by assignee, when assignee value is too long.
Should respond with an error message. +
*HTTP Method type*: *GET* +
...
****
*Description*: Update a non-existing todo.
Should respond with an error message. +
*HTTP Method type*: *PUT* +
...
****
*Description*: Delete a non-existing todo.
Should respond with an error message. +
*HTTP Method type*: *DELETE* +
*Endpoint*: http://localhost:8080/todos/37 +
*Request Payload*:
...
A Swagger JSON file is often needed specially to upload to an API store.
Link to copy the JSON content: +
